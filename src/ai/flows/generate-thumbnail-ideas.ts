...
const generateThumbnailIdeasFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const imageGenPromptText1 = `Generate a viral YouTube thumbnail with a high-energy, high-contrast style. The image must be in cinematic quality, 8k, photorealistic, and highly detailed. The image must be in a 16:9 landscape aspect ratio. The video is about: "${input.videoTheme}". The main subject should be based on the character/subject in the reference image. The generated image should be visually striking, with high contrast, vibrant colors, and an exaggerated expression on the subject's face. The image must prominently feature the text "${textIdeas.overlayText}" using a bold, impactful, and easy-to-read font (like 'Komika Axis').`;
    const imageGenPromptText2 = `Generate a second, different version of a viral YouTube thumbnail with a high-energy, high-contrast style. The image must be in cinematic quality, 8k, photorealistic, and highly detailed. The image must be in a 16:9 landscape aspect ratio. The video is about: "${input.videoTheme}". The subject is based on the character/subject in the reference image. Make this version more dramatic or use a different angle, maintaining the high-energy, high-contrast style. The image must prominently feature the text "${textIdeas.overlayText}" using a bold, impactful, and easy-to-read font (like 'Komika Axis'), perhaps with a different color or placement.`;
