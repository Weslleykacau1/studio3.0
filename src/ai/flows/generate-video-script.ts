...
const markdownPromptInstructions = `Your task is to generate the content for the 'markdownPrompt' field. This content should be a comprehensive and detailed prompt in **Markdown format** for a screenwriter AI. This prompt must instruct the AI to create a video script using Markdown headings, lists, and bold/italic text for structure. The prompt must incorporate all the specifications provided in the context above. The dialogue in the final script must be in **Brazilian Portuguese**, matching the influencer's accent: {{{influencerAccent}}}. O diálogo deve ser direto ao ponto, sem necessidade de o influenciador se apresentar. **Crucially, the dialogue should include emotional cues in English (e.g., in parentheses) and emphasize key words or phrases.** For example: "(gasping) Eu não posso acreditar que finalmente consegui!". All other descriptive parts of the script, like visual descriptions, should be in English.`;
