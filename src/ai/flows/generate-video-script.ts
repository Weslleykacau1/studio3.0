...
const jsonPromptInstructions = `Your task is to generate the content for the 'prompt' field. This content should be a comprehensive set of instructions for a screenwriter AI. These instructions must command the AI to generate a video script as a single JSON object with a specific structure: { "title": "...", "synopsis": "...", "script": [ { "timecode": "...", "visuals": "...", "dialogue": "...", "sfx": "...", "text_overlay": "..." } ] }. The instructions you create must incorporate all the specifications provided in the context above. The "dialogue" in the final script's JSON must be in **Brazilian Portuguese**, matching the influencer's accent: {{{influencerAccent}}}. **Crucially, the dialogue should include emotional cues and emphasize key words or phrases to guide the influencer's performance.** For example: "(surpreso) Uau, eu *não* acredito nisso!". All other fields in the script JSON should be in English.`;
const markdownPromptInstructions = `Your task is to generate the content for the 'markdownPrompt' field. This content should be a comprehensive and detailed prompt in **Markdown format** for a screenwriter AI. This prompt must instruct the AI to create a video script using Markdown headings, lists, and bold/italic text for structure. The prompt must incorporate all the specifications provided in the context above. The dialogue in the final script must be in **Brazilian Portuguese**, matching the influencer's accent: {{{influencerAccent}}}. **Crucially, the dialogue should include emotional cues (e.g., in parentheses) and emphasize key words or phrases (e.g., using asterisks for italics or bold).** For example: "(ofegante) Eu não posso acreditar que *finalmente* consegui!". All other descriptive parts of the script should be in English.`;
